...
gmf.TreeManager = function($timeout, gettextCatalog, ngeoLayerHelper,
  ...
  /**
   * Number of group to add to the layertree during one single Angular
   * digest loop.
   * @type {number}
   * @public
   */
...
gmf.TreeManager.prototype.addFirstLevelGroup_ = function(group) {
  ...
  // Add group in the list of groups to add And be sure that the counter of
  // group to add is reset.
  ...
  // loop. The purpose of this is to preserve a coherent order between the
  ...
  this.promiseForGroupsToAddInThisDigestLoop_ = this.$timeout_(function() {
    ...
    // Reset the groups an the promise state. Don't reset the
    // numberOfGroupsToAddInThisDigestLoop, it must persist because the layers
    // will added into the map after that the layertree template is generated (
    // so in the next angular loop).
