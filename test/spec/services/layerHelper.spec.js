...
describe('ngeo.LayerHelper', function() {
  ...
  it('Get index of a layer', function() {
    ...
    expect(ngeoLayerHelper.getLayerIndex(layer)).toBe(-1);
    map.addLayer(layer);
    expect(ngeoLayerHelper.getLayerIndex(layer)).toBe(0);
  });
  ...
  it('Add a layer on the map', function() {
    layer = ngeoLayerHelper.createBasicWMSLayer('', 'testLayer');
    expect(ngeoLayerHelper.addLayerToMap(layer)).toBe(true);
    expect(ngeoLayerHelper.addLayerToMap(layer)).toBe(false);
  });

  it('Remove a layer from the map', function() {
    layer = ngeoLayerHelper.createBasicWMSLayer('', 'testLayer');
    map.addLayer(layer);
    expect(ngeoLayerHelper.removeLayerFromMap(layer)).toBe(true);
    expect(ngeoLayerHelper.removeLayerFromMap(layer)).toBe(false);
  });

  it('Add and remove multiple layers', function() {
    layer = ngeoLayerHelper.createBasicWMSLayer('', 'testLayer');
    ngeoLayerHelper.moveInOutLayers([layer], true);
    expect(map.getLayers().array_.length).toBe(1);
    ngeoLayerHelper.moveInOutLayers([layer], false);
